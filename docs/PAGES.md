...
- **출처 칩 (FEAT-041, TS-100)**: 청크의 노션 원본 URL(공개 워크스페이스)로 새 탭 연결 — 새 탭 차단 환경(웹뷰·인앱)은 같은 탭 이동 폴백. 주의: window.open 에 "noopener" 를 features 로 주면 성공해도 null 반환 → 이중 이동 버그(2026-08 회귀) — features 없이 열고 w.opener=null 로 끊는다. URL 이 https 가 아니면 비공개 disabled 칩.
...
1. **커리어 타임라인 (이력서 단일 소스, 2026-07 중복 fix)** — 노션 **이력서**(career 청크)만
   ...
   - 행 공통: 좌측 회사명(또는 "자체 프로젝트")·직함(있을 때만)·기간, `###` 프로젝트 제목 그룹 + 불릿.
   - 자체 프로젝트 항목은 역할 표기 없음 (`| 자체 프로젝트` callout 단독 — role 폴백 제거).
   - 진행 중 항목은 brand 점.
   - ~~project 청크(프로젝트 DB) 병합~~ 제거 — 프로젝트 DB에 운영 컨테이너 row("대화형 포트폴리오")가
     함께 잡혀 중복 노출되던 문제의 원천 차단 (ADR-032).
...
### 와이어프레임 (모바일)
- 항목 헤더(회사명·직함·기간)를 콘텐츠 상단 인라인 렌더 (좌측 컬럼·점 숨김)
- 회사 → 프로젝트 그룹 vertical stack

### 데이터
- 커리어 타임라인·학력·자격증: **이력서 페이지 단일 소스** (career 청크, sync-notion 시점).
- 보유 스킬: skill 청크 (SkillsGrid).
- project 청크는 이 페이지에서 미사용 (RAG 채팅 답변용으로만 유지).

### 인터랙션
- 정적 타임라인 (카드/필터/노션 링크 없음 — 2026-07 개편으로 CategoryFilter·ProjectCard 제거).

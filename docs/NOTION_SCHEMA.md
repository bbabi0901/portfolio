...
본 프로젝트는 **`기록` 페이지를 단일 진실 소스(SSoT)** 로 삼는다. 자기소개·이력서·프로젝트 DB·스터디 DB·트러블슈팅 DB(TBD) 모두 기록 하위에 둔다. Q&A 피드백·Contact 같은 운영 DB는 프로젝트 DB의 "대화형 포트폴리오" row 하위에 위치.

전체 트리·ID·역할 표는 [`NOTION_STRUCTURE.md`](./NOTION_STRUCTURE.md#페이지-트리) 참조.
...
- 주의: 운영 컨테이너 row("대화형 포트폴리오" — Q&A 피드백·Contact DB 보관용)도 이 필터를
  통과해 project 청크로 들어간다. **커리어 페이지는 project 청크를 쓰지 않으므로**(ADR-032,
  이력서 단일 소스) 노출 문제는 없고, RAG 답변용으로만 남는다.

### 이력서 "자체 프로젝트 (Personal Project)" 섹션 포맷 (ADR-032)
커리어 타임라인은 이력서 단일 소스다. 자체 프로젝트 항목은 회사 경력과 같은 블록 구조를 따라야
`parseCareerMarkdown` 이 타임라인 엔트리로 인식한다:

1. `callout` — `| 자체 프로젝트` (역할 라인 없음). **아이콘은 built-in**(briefcase 등)만 사용 —
   이모지 아이콘은 notion-to-md 마크다운에 텍스트로 노출돼 `> |` 패턴 파싱을 깨뜨린다.
2. `quote` — 기간 (`2026.05 - 현재` 형식, `현재` 포함 시 진행 중 표시)
3. `divider`
4. `heading_3` — 프로젝트명 + `bulleted_list_item` 성과/특징 불릿

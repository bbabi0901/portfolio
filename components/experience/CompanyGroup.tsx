...
export function CompanyGroup({ group, className }: CompanyGroupProps) {
  return (
    ...
    >
      <header className="border-line-subtle flex flex-col gap-1 border-b pb-2">
        <h2 className="text-foreground text-lg font-medium">{group.company}</h2>
        {group.period ? <p className="text-subtle text-xs">{group.period}</p> : null}
      </header>
